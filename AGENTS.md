...
- `tests/run_tests.gd` — 常驻回归测试（28 项：目标函数/BO 收敛/GP 方差/指标聚合/历史存储/图表几何/微调检测/场景实例化/平坦检测），运行 `--headless --script res://tests/run_tests.gd`
...
- **回归测试**: `D:\godotstudy\Godot_v4.7.1-stable_win64.exe\Godot_v4.7.1-stable_win64_console.exe --headless --path "." --script res://tests/run_tests.gd`（28 项常驻用例）
